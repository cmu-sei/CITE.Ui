...
export class ReportComponent implements OnDestroy {
    ...
    (this.evaluationQuery.selectActive() as Observable<Evaluation>)
      ...
      .subscribe((active) => {
        if (active) {
          ...
          this.displaying = this.uiDataService
            .getSubmissionType(active.id)
            .toLowerCase();
        }
      });
  ...
  selectDisplayedSubmissions() {
    let validSubmissions = Object.assign(
      [],
      this.submissionList
    ) as PopulatedSubmission[];
    validSubmissions = validSubmissions.filter(
      (vs) => vs.submissionType.toLowerCase() === this.displaying
    );
    ...
    validSubmissions.forEach((vs) => {
      const hasScores =
        ...
        this.selectedScoringModel.scoringCategories.some(
          (sc) =>
            +vs.moveNumber >= +sc.moveNumberFirstDisplay &&
            +vs.moveNumber <= +sc.moveNumberLastDisplay
        );
      if (
        hasScores &&
        +vs.moveNumber <= +this.selectedEvaluation.currentMoveNumber
      ) {
        displayedSubmissions.push(vs);
      }
    });
    displayedSubmissions = displayedSubmissions.sort((a, b) =>
